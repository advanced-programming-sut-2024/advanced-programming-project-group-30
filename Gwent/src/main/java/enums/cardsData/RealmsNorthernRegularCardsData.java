...
public enum RealmsNorthernRegularCardsData {
    ...
    private static RegularCard createCard(RealmsNorthernRegularCardsData data, Faction faction) {
        RegularCardsAbility ability = RegularCardsAbility.createNewAbilityByName(data.abilityName);
        return new RegularCard(data.name, data.explanation, faction, data.isHero, data.point, ability, data.cardPositionType);
    }

    public static ArrayList<RegularCard> getAllRegularCard(Faction faction) {
        ArrayList<RegularCard> regularCards = new ArrayList<>();
        for (RealmsNorthernRegularCardsData data : RealmsNorthernRegularCardsData.values())
